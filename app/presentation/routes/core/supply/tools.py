...
# EXCEPTION: Direct ORM usage allowed for simple GET operations on Tool
...
def list():
    ...
    if manufacturer:
        query = query.filter(ToolDefinition.manufacturer.ilike(f'%{manufacturer}%'))
    ...
    # Get filter options
    tool_types = db.session.query(ToolDefinition.tool_type).distinct().all()
    tool_types = [tt[0] for tt in tool_types if tt[0]]
    
    manufacturers = db.session.query(ToolDefinition.manufacturer).distinct().all()
...
def detail(tool_id):
    ...
    tool = ToolDefinition.query.get_or_404(tool_id)
...
# EXCEPTION: Direct ORM usage allowed for simple EDIT operations on Tool

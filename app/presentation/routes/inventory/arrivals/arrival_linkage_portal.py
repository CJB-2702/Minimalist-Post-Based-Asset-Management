...
def register_arrival_linkage_routes(inventory_bp):
    """Register arrival linkage portal routes"""
    ...
    def arrival_linkage_link_to_po_line(package_id):
        ...
        context = PackageArrivalContext(package_id)
        manager = context.linkage_manager
        success, message = manager.link_arrival_to_po_line(
            part_arrival_id, po_line_id, quantity_to_link, current_user.id
        )
    ...
    def arrival_linkage_unlink_from_po_line(package_id):
        ...
        context = PackageArrivalContext(package_id)
        manager = context.linkage_manager
        success, message = manager.unlink_arrival_from_po_line(part_arrival_id, current_user.id)
